...
#define LED_BRIGHTNESS 20 // from 0 to 255
...
void startDissipating() { 
  ...
  for(int j = 0; j<5; j++) { 
    ...
    for(int i = 0; i<NUM_LEDS; i++ ) { 
        strip.setPixelColor(i, strip.Color(0,LED_BRIGHTNESS,0)); 
        strip.show(); 
        delay(20); 
    }
  }
...
void updateLedStrip() { 
  ...
  for(uint16_t i=0; i<NUM_LEDS*2; i++) {
    if(i<=currentLevelPixel) strip.setPixelColor(i/2, strip.Color((i%2==0) ? LED_BRIGHTNESS : 0, LED_BRIGHTNESS, 0));
